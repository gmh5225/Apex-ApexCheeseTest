...
NTSTATUS CallbackREAD(PREQUEST_READ args)
{
	...
	if (args->bPhysicalMem) {
		...
		if (NT_SUCCESS(status)) {
			...
			/*if (NT_SUCCESS(status)) {
				RtlCopyMemory(args->Dest, &targetAddress, sizeof(targetAddress));
			}*/
...
NTSTATUS CallbackGETSTRING(PREQUEST_GETSTRING args)
{
	if (((PBYTE)args->Src + args->Size < (PBYTE)args->Src) ||
		((PBYTE)args->Dest + args->Size < (PBYTE)args->Dest) ||
		((PVOID)((PBYTE)args->Src + args->Size) > MM_HIGHEST_USER_ADDRESS) ||
	...
	if (args->bPhysicalMem) {
		...
		if (NT_SUCCESS(status)) {
			...
			char name[32];
			SIZE_T outSize = 0;
			status = (MmCopyVirtualMemory)(process, args->Src, (PsGetCurrentProcess)(), &name, sizeof(name), KernelMode, &outSize);
			(ObfDereferenceObject)(process);

			if (NT_SUCCESS(status)) {
				char* outAddress = name;
				unsigned char myByte[60] = { };
				...
				while (outAddress[loop] != '\0')
				{
					myByte[i++] = outAddress[loop++];
				...
				RtlCopyMemory(args->OutDest, &myByte, sizeof(myByte));
			}
		}
		return status;
	}
...
NTSTATUS CallbackGETMATRIX(PREQUEST_GETMATRIX args)
{
	if (((PBYTE)args->Src + args->Size < (PBYTE)args->Src) ||
		((PBYTE)args->Dest + args->Size < (PBYTE)args->Dest) ||
		((PVOID)((PBYTE)args->Src + args->Size) > MM_HIGHEST_USER_ADDRESS) ||
	...
	if (args->bPhysicalMem) {
		...
		if (NT_SUCCESS(status)) {
			...
			status = (MmCopyVirtualMemory)(process, args->Src, (PsGetCurrentProcess)(), &myFloat, sizeof(myFloat), KernelMode, &outSize);
			(ObfDereferenceObject)(process);

			if (NT_SUCCESS(status)) {
				RtlCopyMemory(args->Dest, &myFloat, sizeof(myFloat));
			}
		}
		return status;
	}
